        ...
        if st.button('Generate Power Units Overview', use_container_width=True, disabled=disable_units):
            try:
                ...
                for uploaded_file in plant_files:
                    chunk = get_units_per_plant(plant_file_path=uploaded_file, plant_name=uploaded_file.name.replace('.xlsx', ''))
                    if chunk is not None and not chunk.is_empty():
                        master_units = chunk if master_units is None else pl.concat([master_units, chunk], how="diagonal_relaxed")
