...
def render_regionalization_tab():
    ...
    if 'plant_dfs' not in st.session_state:
        st.session_state.plant_dfs = []
    df_regio = None
            ...
            if st.button(
                ...
                plant_dfs = {key: 
                    v['df'].with_columns([pl.col(pl.Int64, pl.Float64, pl.Null).cast(pl.Float64),
                                          pl.lit(v['mapping_row']['DSH plant name']).alias('Plant name'),
                                          pl.lit(v['mapping_row']['Sector']).alias('Sector'),
                                          pl.lit(v['mapping_row']['Cluster']).alias('Cluster'),                                     
                                          ])
                    for key, v in only_scenario_data['data'].items()
                    if not v['df'].is_empty()
                }

                if plant_dfs:
                    st.session_state.plant_dfs = plant_dfs
                else:
                    
                    st.warning("No plant data available to aggregate.")
            # st.write(st.session_state.plant_dfs)
            # st.dataframe(st.session_state.all_data_regio['527be9a2-5258-4cda-89cb-b485207a42d0']['df'])
        st.write(st.session_state.all_data_regio['48fb5231-1d91-4906-97e7-8c44332b0637'])
        # st.write(st.session_state.plant_dfs[0])
        # df = pl.DataFrame()
        # for plant in st.session_state.all_data_regio:
        #     v['df'].with_columns()

        result = build_regionalization_electricity_demand(st.session_state.plant_dfs, res)

        st.dataframe(result)
        print(result)

        if st.button("Generate Regionalization File"):
            try:
                categories_df = get_overview_categories()
                excel_bytes = build_regionalization_excel(st.session_state.plant_dfs, res, categories_df)

                st.download_button(
                    label="Download regionalization.xlsx",
                    data=excel_bytes,
                    file_name=f"regionalization_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
