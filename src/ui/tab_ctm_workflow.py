...
from CONNECT_CTM.utils.utils import check_plants_in_mapping
...
def _plant_files_cache_key(plant_files) -> str:
    """Cheap fingerprint -- avoids re-parsing every workbook on every rerun."""
    return "|".join(f"{f.name}:{f.size}" for f in plant_files)


@st.cache_data(show_spinner=False)
def _cached_extract_scenario_years_multi(_plant_files, cache_key: str):
    return extract_scenario_years_multi(_plant_files)


@st.cache_data(show_spinner=False)
def _build_inputs_zip(all_inputs_by_key: dict) -> bytes:
    """Plain dict of str/tuple/dict -- Streamlit can hash this directly,
    no manual cache key needed (unlike the plant_files/DataFrame cases)."""
    zip_buffer = io.BytesIO()
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
        for (scenario, year), inputs in all_inputs_by_key.items():
...
def render_ctm_workflow_tab():
    ...
    with st.expander("Step 1: Upload Data Files", expanded=False):
        ...
        with col1:
            ...
            if plant_files:
                ...
                if st.button(
                    'Aggregate the data for viz', use_container_width=True, key='get_only_data',
        ...
        with col3:
            ...
            if curves_file:
                ...
                with col33:
                    flexibility_sheet = st.text_input('Flexibility sheet name', value='Flexibility', width=200)
                ...
                if st.button('Load and process cluster/sector curves file', type='primary'):
                    try:
                        ...
                        st.session_state.flexibility_curves_df = pl.read_excel(curves_file, sheet_name=flexibility_sheet)
    ...
    with st.expander("Step 2: Settings", expanded=False):
        ...
        with col2:
            with st.container(border=True):
                if plant_files:
                    scenarios, years = _cached_extract_scenario_years_multi(
                        plant_files, _plant_files_cache_key(plant_files)
                    )
                    ref_year = years[0]
                    years = years[1:]
                else:
    ...
    with st.expander("Step 3: Push to CTM", expanded=False):
            ...
            try:
                with st.spinner("Processing..."):
                    st.session_state.result = push_aggregated_by_scenario_year(
                        ...
                        cluster_sector_flexibility=st.session_state.flexibility_curves_df,
        ...
        if st.session_state.result is not None:
            ...
            if st.session_state.result.get("all_inputs"):
                zip_bytes = _build_inputs_zip(st.session_state.result["all_inputs"])
    ...
    with st.expander("Step 4: Couple to ETM", expanded=False):
        ...
        if not st.session_state.ctm_sessions:
            st.warning("⚠️ CTM session IDs not set. Add them in Step 2.")
        else:
            st.success("✓ CTM IDs ready")

        # st.info('Using ETM LIVE version (#latest)')
        etm_version = st.selectbox(label='Select ETM version. Type in a different version if it is not in the dropdown.',
                                   options=['LIVE', 'BETA', '2025-01'], accept_new_options=True)
        ...
        if st.button("Couple to ETM", type="primary"):
            if st.session_state.etm_token and st.session_state.etm_session_json:
                ...
                res_push, logs = couple_all_sessions_to_etm(
                    ...
                    use_ctm_beta=st.session_state.use_beta,
                    etm_token=st.session_state.etm_token,
                    max_retries=max_retries,
                    retry_delay_seconds=1.0,
                    log_container=log_container,
                    etm_version=etm_version
                )
