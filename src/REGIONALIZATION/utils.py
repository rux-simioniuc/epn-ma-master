...
def build_regionalization_electricity_demand(
    ...
    for plant_id, df in plant_data.items():
        demand = df.filter(pl.col("Flow type") == "demand").select([
            "Scenario", "Year", "Electricity", "Sector", "Cluster", "Plant name",
        ])
        if demand.is_empty():
            continue
        demand = demand.with_columns(pl.lit(plant_id).alias("Plant identifier"))
        rows.append(demand)
...
def build_column_groups(tidy_df: pl.DataFrame, categories_df: pl.DataFrame) -> list[dict]:
    """
    Cross-join every (Scenario, Year) found in the data with every row of
    categories_overview.csv -- so ALL categories appear as columns, not just
    ones with matching plant data.
    """
    category_groups = load_category_groups(categories_df)

    scenario_years = tidy_df.select(["Scenario", "Year"]).unique()

    all_groups = scenario_years.join(category_groups, how="cross")

    data_sectors = set(tidy_df.select("Sector").unique().to_series().to_list())
    category_sectors = set(category_groups.select("Sector").unique().to_series().to_list())
    uncovered = data_sectors - category_sectors
    if uncovered:
        print(f"[WARN] Sector(s) in plant data with no matching category row (will get no column): {sorted(uncovered)}")

    return all_groups.sort(["Scenario", "Year", "Sector", "Level"]).to_dicts()
...
def write_regionalization_excel(tidy_df: pl.DataFrame, categories_df: pl.DataFrame) -> bytes:
    """
    Write the tidy electricity-demand-by-site table to a merged-header Excel
    file matching the target regionalization layout:

        (row) SCENARIO           -- not merged
        (row) YEAR                -- not merged
        (row) Sector               -- merged across matching runs
        (row) PtH                   -- "pth" where the category is PtH-flagged, else blank
        (row) CARRIER (PJ)
        (row) LEVEL
        (row) TYPE
        (row) ID  /  Site | Buurtcode | Postcode

    KNOWN GAPS (flagged, not silently guessed at):
      - Postcode: not in the location df yet -- written blank for every row.
      - PtH-flagged columns are always left blank: we have no data source
        that splits demand into PtH vs. non-PtH yet, so filling them with
        the same value as the non-PtH column would double-count demand.
    """
    if tidy_df.is_empty():
        raise ValueError("tidy_df is empty -- nothing to write.")

    col_groups = build_column_groups(tidy_df, categories_df)

    sites = tidy_df.select(["Plant name", "buurtcode", "buurtnaam"]).unique().sort("Plant name")

    value_lookup = {
        (r["Plant name"], r["Scenario"], r["Year"], r["Sector"], r["Level"]): r["Electricity"]
        for r in tidy_df.to_dicts()
    }

    buffer = io.BytesIO()
    with xlsxwriter.Workbook(buffer) as workbook:
        for carrier in categories_df.select(pl.col('Energy carrier')).unique().to_series().to_list():
            ...
            SITE_COL = 0    # column A
            LABEL_COL = SITE_COL + 3   # column I (0-indexed)
            
            BUURT_COL = SITE_COL + 1   # column E
            POST_COL = SITE_COL + 2    # column F
            DATA_START_COL = LABEL_COL + 1  # column J
            ...
            ROW_LEVEL = 5
            ROW_TYPE = 6
            ROW_ID = 7  # also the Site / Buurtcode / Postcode label row
            ...
            ws.write(ROW_CARRIER, LABEL_COL, "CARRIER (PJ)", label_format)
            ...
            ws.write(ROW_ID, SITE_COL, "Site", site_header_format)
            ws.write(ROW_ID, BUURT_COL, "Buurtcode", site_header_format)
            ws.write(ROW_ID, POST_COL, "Postcode", site_header_format)  # GAP: no data source for this yet

            # SCENARIO / YEAR: explicitly no merging, per request
            _write_plain_header(ws, ROW_SCENARIO, DATA_START_COL, col_groups, "Scenario", header_format)
            _write_plain_header(ws, ROW_YEAR, DATA_START_COL, col_groups, "Year", header_format)

            # Sector: merged across contiguous runs (col_groups is sorted for this)
            _write_grouped_header(ws, ROW_SECTOR, DATA_START_COL, col_groups, "Sector", header_format,
                                group_keys=["Scenario", "Year", "Sector"])

            for i, group in enumerate(col_groups):
                col = DATA_START_COL + i
                ws.write(ROW_PTH, col, "pth" if group["PtH"] else "", header_format)
                ws.write(ROW_CARRIER, col, group["Carrier"], header_format)
                ws.write(ROW_LEVEL, col, group["Level"], header_format)
                ws.write(ROW_TYPE, col, group["Type"], header_format)
                ws.write(ROW_ID, col, group["ID"], header_format)

            for r, site in enumerate(sites.to_dicts()):
                row = DATA_START_ROW + r
                ws.write(row, SITE_COL, site["Plant name"])
                ws.write(row, BUURT_COL, site["buurtcode"])
                ws.write(row, POST_COL, None)  # GAP: Postcode not available yet

                for i, group in enumerate(col_groups):
                    col = DATA_START_COL + i

                    if group["PtH"]:
                        continue  # GAP: no PtH-split data source -- leave blank

                    value = value_lookup.get(
                        (site["Plant name"], group["Scenario"], group["Year"], group["Sector"], group["Level"])
                    )
...
def build_regionalization_excel(
    ...
    tidy_df = build_regionalization_electricity_demand(plant_data, location_df)
    return write_regionalization_excel(tidy_df, categories_df)
