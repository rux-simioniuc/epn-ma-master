...
from .utils.constants import SCENARIO_YEARS, REFERENCE_YEAR, EMISSION_COLS_ORDER, UTILITY_COLS_ORDER, ALL_SCENARIOS, PATH_PERSISTENT_FILES


# def set_empty_to_zero(inputs: dict) -> dict:
#     return {
#         key: 0 if value is None or value == "" else value
...
def get_custom_ctm_inputs() -> Dict[str, str]:
    ...
    return {
        ...
        "bp&&waste_gas_to_chp_first": "1",
        "exxonmobil&&waste_gas_to_chp_first": "1",
        "gunvor&&waste_gas_to_chp_first": "1",
        "vpr_energy&&waste_gas_to_chp_first": "1",
        "zeeland_refinery&&waste_gas_to_chp_first": "1",
...
def _map_energy_row(row_data: dict, energy_cols: List[str], flow_type: str, keep_empty_and_zero: bool = False) -> Dict[str, str]:
...
def _map_emission_row(row_data: dict, emission_cols: List[str], keep_empty_and_zero: bool = False) -> Dict[str, str]:
...
def construct_site_inputs(
    ...
    for ctm_col, val in _map_energy_row(row_data, energy_cols, flow_type, keep_zero).items():
        inputs[build_site_data_key(sector, cluster, site, ctm_col)] = val

    if flow_type == "production":
        for ctm_col, val in _map_emission_row(row_data, emission_cols, keep_zero).items():
            inputs[build_site_data_key(sector, cluster, site, ctm_col)] = val
...
def load_all_plants_scenario_data(
    ...
    for wb_source, file_bytes in workbooks:
        ...
        try:
            plant_match = mapping_df.filter(
                pl.col("DSH plant name").str.to_lowercase() == plant_name.lower()
            )
            ...
            plant_key = f"{plant_id}__{plant_name}"
            ...
            if not scenario_df.is_empty():
                ...
                try:
                    production_df  = read_production_table(workbook_path=str(wb_path))[0]
                    if not production_df.is_empty():
                        all_scenario_data[plant_key]["production_df"] = production_df
                ...
                try:
                    flexibility_df = read_production_table(workbook_path=str(wb_path), sheet_name='Flexibility')[0]
                    if not flexibility_df.is_empty():
                        flexibility_df = flexibility_df.filter(pl.col('Year').is_not_null())
                        all_scenario_data[plant_key]["flexibility_df"] = flexibility_df
    ...
    for plant_key, plant_data in all_scenario_data.items():
        for row in plant_data["df"].to_dicts():
            key = (row["Scenario"], str(row["Year"]))
            scenario_year_groups.setdefault(key, []).append((plant_key, row))
        ...
        try:
            if is_bottom_up:
                ...
                plant_inputs = construct_bottom_up_inputs(
                    ...
                    flow_type=flow_type, logs=logs, transformation=transformation, keep_zero=keep_zero
                )
                ...
                plant_inputs = construct_custom_site_inputs(
                    ...
                    row_data=row_data, emission_cols=emission_cols, energy_cols=energy_cols,
                    flow_type=flow_type, logs=logs, latitude=latitude, longitude=longitude, keep_zero=keep_zero
                )
                inputs.update(plant_inputs)
...
def build_sector_cluster_sites(cluster_sector_data: pl.DataFrame) -> Tuple[Dict, List]:
    ...
    for row in cluster_sector_data.to_dicts():
        ...
        if 'co2' in utility or 'other_greenhouse_gas_emissions_production' in utility:
            inputs[f"{sector}&&sector&&{utility}"] = str(value)
        else:
...
def build_ctm_inputs_from_prod_flex(
    sector: str,
    cluster: str,
    site: Optional[str],
    ...
    This functions handles both produciton and flexibility tables; set is_production flag to indicate which is used
    """
    logs = []
    inputs = {}
    cluster = fix_string(cluster)
    ...
    if is_production:
        ...
        # means it's flexibility
        column_map = {
            "Capacity": "hybrid_heat_capacity",
            "FLH": "hybrid_heat_flh",
        }
...
def _build_production_flex_for_plant(
    plant_info: dict,
    mapping_row: dict,
    production_df: pl.DataFrame,
    logs: list,
    ...
    if mapping_row["Bottom-up"]:
        site_kwargs = {"site": ctm_api_name.split("&&")[0], "is_bottom_up": True}
    elif mapping_row["New site"]:
        site_kwargs = {"site": ctm_api_name.split("&&")[0], "is_custom": True}
    else:
        parts = ctm_api_name.split("&&")
        if len(parts) < 3:
            return {}, logs
        site_kwargs = {
            "sector": parts[0], "cluster": parts[1], "site": parts[2],
        }

    site_kwargs.setdefault("sector", fix_string(mapping_row["Sector"]))
    site_kwargs.setdefault("cluster", fix_string(mapping_row["Cluster"]))

    try:
        prod_inputs, prod_logs = build_ctm_inputs_from_prod_flex(
            production_df=production_df, is_production=is_production, **site_kwargs,
        )
        logs.extend(prod_logs)
        return prod_inputs, logs
    except Exception as e:
        kind = "Production" if is_production else "Flexibility"
        logs.append(f"  [ERROR] {kind} for {plant_info['name']}: {e}")
        return {}, logs
...
def process_plants_for_scenario_year(
    ...
    for plant_id, row in plants_in_year:
        ...
        prod_inputs, logs = _build_production_flex_for_plant(
        ...
        filtered_flex = plant_info["flexibility_df"].filter(
            (pl.col("Scenario") == scenario) & (pl.col("Year") == year)
        )
        if filtered_flex.is_empty():
            continue

        flex_inputs, logs = _build_production_flex_for_plant(
            plant_info=plant_info,
            mapping_row=plant_info["mapping_row"],
            production_df=filtered_flex,
            logs=logs,
            is_production=False
        )
...
def session_file_exists(session_id: str) -> bool:
    os.makedirs(PATH_PERSISTENT_FILES, exist_ok=True)
    path = os.path.join(PATH_PERSISTENT_FILES, f'{session_id}.json')
    if os.path.exists(path):
        return True
    print(f'No persistent file found for session {session_id}')
    return False


def get_persistent_inputs(session_id: str) -> dict:
    path = os.path.join(PATH_PERSISTENT_FILES, f'{session_id}.json')
    try:
        with open(path, 'r') as f:
            return json.load(f)
    except (json.JSONDecodeError, OSError) as e:
        print(f'[ERR] Could not read persistent file for {session_id}: {e}')
        return {}
    
def _values_equal(a, b) -> bool:
    """Values are stored as strings throughout the push pipeline -- compare
    as strings explicitly rather than relying on Python's == across types."""
    return str(a) == str(b)


def get_relevant_inputs(session_id: str, all_inputs: dict) -> tuple[dict, dict]:
    """
    Returns (delta_to_push, full_state_to_persist).

    delta_to_push: only what actually needs sending to CTM this time --
      - a key not seen before, with a nonzero value
      - a key whose value changed from what's on file (this naturally
        covers "push 0 if the previous value was nonzero": old="5", new="0"
        are different strings, so it gets pushed)
    full_state_to_persist: old history overlaid with this push, to diff
      against next time.
    """
    if not session_file_exists(session_id):
        # if file does not exist, filter only non zero values
        delta = {k: v for k, v in all_inputs.items() if not _values_equal(v, "0")}
        return delta, delta

    old_inputs = get_persistent_inputs(session_id)
    if not old_inputs:
        print('Persistent file empty or could not be read')
        delta = {k: v for k, v in all_inputs.items() if not _values_equal(v, "0")}
        return delta, delta
    ...
    for key, value in all_inputs.items():
        if key not in old_inputs:
            if not _values_equal(value, "0"):  # skip brand-new zero values
                delta[key] = value
        elif not _values_equal(value, old_inputs[key]):
            delta[key] = value

    full_state = dict(old_inputs)
    full_state.update(delta)

    return delta, full_state


def update_file(inputs: dict, session_id: str):
    os.makedirs(PATH_PERSISTENT_FILES, exist_ok=True)
    path = os.path.join(PATH_PERSISTENT_FILES, f'{session_id}.json')
...
def push_aggregated_by_scenario_year(
    ...
    cluster_sector_flexibility_sheet_name: Optional[str] = None,
    cluster_sector_production: Optional[pl.DataFrame] = None,
    cluster_sector_flexibility: Optional[pl.DataFrame] = None,
    reference_year: int = REFERENCE_YEAR,
    use_beta: bool = True,
    transformation_overrides: Optional[dict] = None,
    ...
    if cluster_sector_file is not None:
        ...
        try:
            ...
            all_errors.append(str(e))
        try:
            log_message("STEP 1D: Loading cluster/sector flexibility curves...")
            if cluster_sector_flexibility is None:
                sector_cluster_flex_df = read_production_table_curves(
                    workbook_path=cluster_sector_file, sheet_name=cluster_sector_flexibility_sheet_name,
                )
            else:
                sector_cluster_flex_df = read_production_table_curves(curves_df=cluster_sector_flexibility)
            log_message(f"  OK Flexibility cluster/sector data: {len(sector_cluster_flex_df)} rows")
        except Exception as e:
            log_message(f"[ERROR] Failed to load cluster-sector flexibility data: {e}")
            all_errors.append(str(e))
    ...
    for (scenario, year), plants_in_year in sorted(scenario_year_groups.items()):
        ...
        if sector_cluster_flex_df is not None:
            filtered_flex = sector_cluster_flex_df.filter(
                (pl.col("Scenario") == scenario) & (pl.col("Year") == year)
            )
            if not filtered_flex.is_empty():
                for row in filtered_flex.to_dicts():
                    flex_inputs, flex_logs = build_ctm_inputs_from_prod_flex(
                        sector=row["Sector"], cluster=row["Cluster"], site=None,
                        production_df=pl.DataFrame(row), is_sector_cluster=True,
                        is_production=False
                    )
                    all_inputs.update(flex_inputs)
                    all_logs.extend(flex_logs)
        ...
        if all_inputs and len(all_inputs) > 1:
            try:
                delta, full_state = get_relevant_inputs(session_id=reuse_sid, all_inputs=all_inputs)
                delta.update(extra_inputs)
                full_state.update(extra_inputs)
                ctm.set_inputs(delta)
                update_file(full_state, reuse_sid)

                sessions_created[scenario_key] = reuse_sid
                log_message(f"  Pushed {len(delta)} total inputs")
                dict_all_inputs[(scenario, year)] = delta
            except Exception as e:
