...
def get_aggregated_curves(
    ...
    curves = curves.with_columns(
        pl.when(pl.col("Netbeheerder").str.to_lowercase() == "emissies")
        .then(pl.lit("cluster"))
    ...
    energidragers_map = {
        ...
        'co2-non-energetic': 'co2_emissions_production',
        'co2': 'co2_emissions_production',
        'non-co2-ghg': 'other_greenhouse_gas_emissions_production',
    }

    sums = curves.with_columns([
        pl.col("Energiedrager").str.to_lowercase().replace(energidragers_map).alias('Utility'),
    ]).drop('Energiedrager')

    sums = sums.group_by(
        ['Cluster', 'Sector', 'Scenario', 'Utility']
    ).agg(
        [pl.col(i).sum() for i in all_years]
    ).sort(['Cluster', 'Sector', 'Scenario', 'Utility'])

    sums = sums.with_columns( pl.lit('demand').alias('Flow type'),)
...
def reshape_cluster_sector_curves(
    ...
    print(long_df.with_columns(pl.col('Year').cast(pl.Utf8)))
