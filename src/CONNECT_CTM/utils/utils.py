...
from .string_utils import normalize_sector_cluster_mapping, fix_column, fix_string
...
def check_plants_in_mapping(plant_files, mapping_df: pl.DataFrame) -> list[str]:
    mapped_plants = mapping_df.select(pl.col('DSH plant name')).to_series().to_list()
    mapped_plants = set(i.lower() for i in mapped_plants if i is not None)
    missing_plants = []
    for plant in plant_files:
        name = Path(plant.name).stem
        if name.lower() not in mapped_plants:
...
def get_units_per_plant(plant_file_path, plant_name):
    ...
    if prod is None or prod.is_empty():
        return None

    prod = prod.select(['Scenario', 'Year', 'Units', 'Capacity'])
    ...
    result = (
        ...
        .with_columns(
            ...
            # zero-capacity scenario/year -> its units' power is 0 too, not
            # the static nameplate value from prod_init
            pl.when((pl.col("Capacity").cast(pl.Float64, strict=False) == 0) | pl.col("Capacity").is_null())
              .then(0.0)
              .otherwise(pl.col("Operating power (MW)"))
              .alias("Operating power (MW)"),
        )
        .select([
            "Plant name",
            "Unit name",
    ...
    wide = wide.select([c for c in wide.columns if (len(c) > 1) and (c != 'null')])

    # normalize every value column to float, 2 decimals
    value_cols = [c for c in wide.columns if c not in ("Plant name", "Unit name")]
    wide = wide.with_columns([
        pl.col(c).cast(pl.Float64).round(2) for c in value_cols
    ])
