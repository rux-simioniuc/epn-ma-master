...
def push_ctm_scenario_to_etm(
    ...
    etm_session: str,
    etm_token: str,
    use_ctm_beta: bool = True,
    etm_version: str=None, 
    log_container=None,
) -> Optional[dict]:
    """
    ...
    try:
        ctm = CTMClient(use_beta=use_ctm_beta)
    ...
    try:
        etm_result = ctm.couple_etm(auth_token=etm_token, etm_session_id=etm_session, etm_version=etm_version)
...
def couple_all_sessions_to_etm(
    ...
    for scenario, year in ctm_sessions.keys():
        ...
        log(f"Pushing {scenario} {year}; CTM {ctm_session} -> ETM {etm_session} version {etm_version}")

        result = None
        for attempt in range(1, max_retries + 1):
            result = push_ctm_scenario_to_etm(ctm_session, etm_session, etm_token, use_ctm_beta=use_ctm_beta, log_container=log_container, etm_version=etm_version)
