import requests
from .utils.ctm_constants import CLUSTERS
...
class CTMClient:
    ...
    def couple_etm(self, 
                   etm_scenario_id: str = None, 
                   etm_session_id: str = None,
                   auth_token: str = None,
        ...
        if etm_version:
            inputs["etm_model_version"] = etm_version

        outputs = ["etm_session_id"] if etm_scenario_id else []
        result = self.set_inputs(inputs, outputs=outputs)
        print(f'Decoupling ETM {etm_session_id} from CTM {self.session_id}')
        inputs = {"etm_coupling_switch": "0"}
        decouple = self.set_inputs(inputs, outputs=outputs)
